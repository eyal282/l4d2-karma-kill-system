...
#define UPDATE_URL    "https://raw.githubusercontent.com/eyal282/l4d2-karma-kill/master/addons/sourcemod/updatefile.txt"
...
static Handle:chargerTimer[MAXPLAYERS]					= INVALID_HANDLE;
static Handle:karmaTime						= INVALID_HANDLE;
static Handle:cvarModeSwitch				= INVALID_HANDLE;
static Handle:cvarPoints				= INVALID_HANDLE;
static Handle:cvarCooldown			= INVALID_HANDLE;
...
public Plugin:myinfo = 
{
	...
	description = " Slows down time and displays stuff on Lethal SI Actions that remove a survivor from the map.",
	version = PLUGIN_VERSION,
	url = "http://forums.alliedmods.net/showthread.php?p=1239108"
}

native PS_SetPoints(client, amount);
...
public OnPluginStart()
{
	...
	HookEvent("jockey_ride_end", event_JockeyKarma, EventHookMode_Pre);
	HookEvent("charger_impact", event_ChargerImpact);
	HookEvent("player_hurt", Event_PlayerHurt, EventHookMode_Post);
	HookEvent("player_hurt", CheckFallInHardRain, EventHookMode_Post);
	HookEvent("player_death", KarmaCharge, EventHookMode_Pre);
	...
	cvarPoints =		CreateConVar("l4d2_karma_charge_points", "69", " Amount of !buy points you get for a karma charge ");
...
public Action:KarmaCharge(Handle:event, const String:name[], bool:dontBroadcast)
{
...
public Action:event_JockeyKarma(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
...
{	
	...
	new Points = GetConVarInt(cvarPoints);
	PS_SetPoints(client, PS_GetPoints(client) + Points);
	
	PrintToChat(client, "\x04[PS]\x01 Karma %s'd!!! + \x05%d\x03 points (Total: \x05%d\x03)", KarmaName, Points, PS_GetPoints(client));
